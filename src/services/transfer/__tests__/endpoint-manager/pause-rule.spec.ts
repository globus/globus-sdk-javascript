...
describe('endpointManager.pauseRule', () => {
  ...
  describe('next', () => {
    test('getAll', async () => {
      const {
        req: { url, method, headers },
      } = await mirror(await pauseRule.next.getAll());
    ...
    test('get', async () => {
      const {
        req: { url, method, headers },
      } = await mirror(await pauseRule.next.get({ pause_rule_id: 'example-pause-rule-id' }));
      expect({ url, method, headers }).toMatchSnapshot();
    });
    ...
    test('update', async () => {
      const {
        ...
        await pauseRule.next.update({
          pause_rule_id: 'example-pause-rule-id',
          request: { data: { message: 'an example message' } },
        }),
      );
      expect({ url, method, headers, json }).toMatchSnapshot();
    });
    ...
    test('remove', async () => {
      const {
        req: { url, method, headers },
      } = await mirror(await pauseRule.next.remove({ pause_rule_id: 'example-pause-rule-id' }));
      expect({ url, method, headers }).toMatchSnapshot();
    });
  });
});
